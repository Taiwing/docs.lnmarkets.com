...
- `timestamp` value is the same as the **`LNM-ACCESS-TIMESTAMP`** header. It <u>**MUST**</u> be number of millisecond since Unix Epoch in UTC and it should be within 30 seconds of our API time.
- `method` <u>**MUST**</u> be uppercase.
- `path` is the request path of the URL, e.g.: _/v1/user_
- `params` is the request body as a JSON string (with no space, no line return) or the request query as URL search params (if there is not data empty string)
...
const signature = createHmac('sha256', secret).update(`${timestamp}${method}${path}${JSON.stringify(params)}`).digest('base64')
```

Bash

```Bash
signature=$(echo -n "$timestamp$method$path$params_as_json_string" | openssl dgst -sha256 -hmac $SECRET -binary | base64 )
```
...
import hmac
import base64
signature = base64.b64encode(hmac.new(secret, timestamp + method + path + data, hashlib.sha256).digest())
```
