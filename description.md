...
curl https://api.lnmarkets.com/v1/user \
  ...
  --header "LNM-ACCESS-SIGNATURE: <the user generated message signature>" \
  --header "LNM-ACCESS-TIMESTAMP: <a timestamp for your request in milliseconds>"
```

- **`LNM-ACCESS-SIGNATURE`** header is generated by creating a **sha256 HMAC** using the secret key on the prehash string `timestamp + method + path + params` (where + represents string concatenation).
...
- `data` is the request body as a JSON String or the request query as URL search params
